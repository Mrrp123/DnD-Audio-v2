...
class SongsDisplay(Widget):
    ...
    def refresh_songs(self):
        ...
        if self.song_search.text == "":
            self.song_list.data = [{"text" : song_data["song"], "song_filepath" : song_data["file"]} for song_data in self.app.music_database]
            return

        self.song_list.data = [{"text" : song_data["song"], "song_filepath" : song_data["file"]} 
                               for song_data in self.app.music_database if self.song_search.text.lower() in song_data["song"].lower()]
...
class DndAudio(App):
    ...
    def on_start(self, *args):
        ...
        if os.path.exists(f"{common_vars.app_folder}/config"):
            ...
            self.set_audioplayer_attr("song_file", self.music_database[0]["file"])
